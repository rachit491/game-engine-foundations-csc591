...
public class GameClient extends PApplet implements EventHandler { 
	...
	//Collision Detection
	CollisionDetection cd = new CollisionDetection(eventManager, phy, userInput, tl, replay);
	...
	public void settings() {
		...
		try {
			...
			while((obj = (Event) in.readObject()) != null && readCount < 10) {
				
				if(((GameObject) obj.getEventArgs().get(0)).getName().contains("player")) {
					p = ((GameObject) obj.getEventArgs().get(0));
					playerid = p.getPID();
				...
				if(((GameObject) obj.getEventArgs().get(0)).getName().contains("platform")) {
					platform.add((GameObject) obj.getEventArgs().get(0));
				}
			...
			//eventManager.setOutputStream(socket.getOutputStream());
	...
	public void draw() {
		...
		if(!tl.getIsPaused()) {			
			...
			//frameRate(this.replay.replayTL.getTickSize());		-- this would be considered cheating so not using
		...
		for(int i=0; i < player.size(); i++) {
			if(player.get(i) != null) {
				...
				if(y.get(i) > height) {
					//Free-Fall = Death
					index = i;
					this.generateEvent(new Event(EventType.DEATH_EVENT, new ArrayList<Object>() {{
	...
	private void updatePlayerMovements(int i) {
		if(i == playerid) {
			...
			this.generateEvent(new Event(EventType.UPDATE_POSITION_EVENT, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("update_position");
			}}));
	...
	void collide(int index, List<GameObject> platform) {
		this.generateEvent(new Event(EventType.COLLISION_EVENT, new ArrayList<Object>() {{
			add(tl.getTimeCycles());
			add(index);
			add(x);
			add(y);
	...
	public void keyPressed() {
		if(key == CODED) {
			this.generateEvent(new Event(EventType.INPUT_EVENT, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("keyPressed");
				add(keyCode);
		...
		if(key == 'a') {	//Record
			this.generateEvent(new Event(EventType.RECORD_SAVE_DATA, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("saveData");
			}}));
			this.generateEvent(new Event(EventType.REPLAY_RECORD, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("replayRecord");
			}}));
		...
		if(key == 's') {	//Stop		
			this.generateEvent(new Event(EventType.REPLAY_STOP, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("replayStop");
			}}));
		...
		if(key == 'd') {	//Normal Replay
			this.generateEvent(new Event(EventType.REPLAY_SAVE_DATA, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("saveData");
			}}));
			this.generateEvent(new Event(EventType.RECORD_RESTORE_DATA, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("saveData");
			}}));
			this.generateEvent(new Event(EventType.REPLAY_NORMAL, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("replayNormal");
			}}));
		...
		if(key == 'f') {	//Fast Replay
			...
			this.generateEvent(new Event(EventType.REPLAY_FF, new ArrayList<Object>() {{
		...
		if(key == 'g') {	//Slow Replay
			...
			this.generateEvent(new Event(EventType.REPLAY_SM, new ArrayList<Object>() {{
				add(replay.replayTL.getTimeCycles());
				add(playerid);
				add("replaySlow");
			}}));
	...
	public void keyReleased() {
		if(key == CODED) {
			this.generateEvent(new Event(EventType.INPUT_EVENT_RELEASE, new ArrayList<Object>() {{
				add(tl.getTimeCycles());
				add(playerid);
				add("keyReleased");
				add(keyCode);
	...
	public void onEvent(Event e) {
		/*if(this.replay.getIsReplaying()) {
			...
			if(e.getEventType() == EventType.DEATH_EVENT) {
				...
				if(!this.replay.getIsReplaying())
					this.generateEvent(new Event(EventType.SPAWN_EVENT, new ArrayList<Object>() {{
						add(tl.getTimeCycles());
						add(e.getClientID());
						add("spawn");
						add(index);
