...
public class GameClient extends PApplet implements Serializable, EventHandler { 
	...
	Physics phy = new Physics(null, replay);
	...
	public void settings() {
		...
		try {
			...
			while((obj = (GameObject) in.readObject()) != null && readCount < 2) {
	...
	public void updateCoordinates(GameObject obj, int playerid) {
		...
		if(!this.sent && this.player.size() > 1) {
	...
	public void renderPlatform(GameObject obj) {
		...
		if(obj != null) {
			...
			//New try using scripts for this!!
			//update(obj);
			//checkBoundaryCollision(obj);
			if(player.size() > 1) {
				ScriptManager.loadScript(System.getProperty("user.dir") + "/src/scripting/ballMovement.js");
				ScriptManager.bindArgument("obj", obj);
				ScriptManager.bindArgument("physics", Components.Physics);
				ScriptManager.bindArgument("player", player);
				ScriptManager.bindArgument("em", eventManager);
				Event e = new Event(EventType.DEATH_EVENT, new ArrayList<Object>() {{
					add(tl.getTimeCycles());
					add(playerid);
					add("death");
					add(playerid);
				}});
				ScriptManager.executeScript("update", GameClient.width, GameClient.height, e);
			}
		}
	}
	...
	public void updatePlayerMovements(int i) {
		if(i == playerid) {
	...
	private void sendPlayerCoordinates() {
		...
		if(userInput.left || userInput.right || userInput.up != 0 || start || !sent) {
			try {
				p.setPID(playerid);
	...
	private void sendPlatformCoordinates() {
		try {
			out.writeObject(platform.get(0));
			out.reset();
		} catch (IOException e) {
			System.out.println("Unable to send coordinates");
		}
	...
	public void onEvent(Event e) {
		/*if(this.replay.getIsReplaying()) {
			...
			if(e.getEventType() == EventType.SPAWN_EVENT) {
				...
				//this.reset(index);
				int index = (int) e.getEventArgs().get(1);
				ScriptManager.loadScript(System.getProperty("user.dir") + "/src/scripting/spawnEventHandler.js");
				ScriptManager.bindArgument("platform", platform);
				ScriptManager.executeScript("handle", index);
			}
